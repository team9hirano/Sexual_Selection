...
#define tend 5000 // 4000 80000 10000
#define mapinitP 0.3
#define initialP 3
#define initialT 1
...
void Map(const char *sex,const char *filename,double K,double initP,int t){
    ...
    if(strcmp(sex, "male") == 0){
        if(strstr(filename, "stmap")) fprintf(gp,"set output 'Twoalleles_hyper_map/MapSt_%s_two_%g_initP_%g.png'\n",sex,K,initP);
        else if(strstr(filename, "intmap")) fprintf(gp,"set output 'Twoalleles_hyper_map/MapInt_%s_two_%g_initP_%g_t_%d.png'\n",sex,K,initP,t);
        else if(strstr(filename, "finmap")) fprintf(gp,"set output 'Twoalleles_hyper_map/MapFin_%s_two_%g_initP_%g.png'\n",sex,K,initP);
...
static inline void genotype(double *sum,int *sexT0,int *sexP0){
    int n;
    double rnd =genrand_real2();
    double total,acc;
    total=0.0;acc=0.0;
    for(n=0;n<4;n++)total+=sum[n];
    if(total==0.0){
        *sexT0=1;*sexP0=1;
        // printf("fault");
        return;
    ...
    else if(rnd<(sum[0]+sum[1]+sum[2])/total){*sexT0=2;*sexP0=1;}
    else {*sexT0=2;*sexP0=2;}
...
//"%d\t%d\t%d\t%d\n",i,j,mgenotype,fgenotype
        ...
        for (k2 = 1; k2 <= 9; k2++) // k2=1; k2<=9; k2++
        {
            ...
            for(i=0;i<LH;i++){
                for(j=0;j<LV;j++){
                    ...
                    else if ((maleT[i][j] == 2 && maleP[i][j] == 1)||(femaleT[i][j] == 2 && femaleP[i][j] == 1))sum3+=1;
                    else if ((maleT[i][j] == 2 && maleP[i][j] == 2)||(femaleT[i][j] == 2 && femaleP[i][j] == 2))sum4+=1;
            ...
            for (i = 0; i < LH; i++)
            {
                ...
                // 途中の図
                //途中の図
                if(t%100==0 && fabs(initP2 - mapinitP) < 1e-12){//t<tend
                    ...
                    for (i=0; i<LH; i++) {
                        for (j=0; j<LV; j++) {
							...
							// if(mgenotype==fgenotype)matgenotypecount+=1;
                            recomap[i*LH + j].i = i;
                            recomap[i*LH + j].j = j;
					...
					// printf("オスとメスの遺伝子型一致割合：%f\n",(double)matgenotypecount/(LH*LV));
                    fclose(snapshot2);
    ...
    gp = popen("gnuplot -persist", "w");
    fprintf(gp, "set terminal png\n");
    fprintf(gp, "set output 'Twoalleles_hyper_new/Three_env_T2P2_K_%f_a1_%f.png'\n", K,a1);
    ...
    for(i=1;i<=9;i++){
        ...
        gp = popen("gnuplot -persist", "w");
        fprintf(gp, "set terminal png\n");
        fprintf(gp, "set output 'Twoalleles_hyper_genoportion_new/Three_env_genoport_K_%f_a1_%f_initP2_%f.png'\n", K,a1,initP2);
        fprintf(gp, "set xrange [0:%d]\n", tend);
