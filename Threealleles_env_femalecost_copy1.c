...
int main(void)
{
    ...
    gp = popen("gnuplot -persist", "w");
    fprintf(gp, "set terminal png\n");
    fprintf(gp, "set output 'Threealleles/Three_env_T3P3_K_%f_V_%f_l_%f_a1_%f_a2_%f.png'\n", K,V,l,a1,a2);
    ...
    gp = popen("gnuplot -persist", "w");
    fprintf(gp, "set terminal png\n");
    fprintf(gp, "set output 'Threealleles/Three_env_T2P2_K_%f_V_%f_l_%f_a1_%f_a2_%f.png'\n", K,V,l,a1,a2);
    fprintf(gp, "set xrange [0:%f]\n", 1.0);
    fprintf(gp, "set xlabel 'T2'\n");
    fprintf(gp, "set yrange [0:%f]\n", 1.0);
    ...
    for(i=1;i<=9;i++){
        ...
        gp = popen("gnuplot -persist", "w");
        fprintf(gp, "set terminal png\n");
        fprintf(gp, "set output 'Threealleles/Three_env_genoport_K_%f_V_%f_l_%f_a1_%f_a2_%f_initP2_%f.png'\n", K,V,l,a1,a2,initP2);
        fprintf(gp, "set xrange [0:%d]\n", tend);
        ...
        //fprintf(gp, "plot \'%s\' using 2:3 with points pointtype 7 lc rgb 'blue' title \"survivalrateV=%f\",\'%s\' using 1:2:($3-$1):($4-$2) with vectors head filled lc rgb 'blue',\'%s\' using 2:3 with points pointtype 7 lc rgb 'red' title \"finalarrival\"\n", data_file4, V, data_file5, data_file6);
        fprintf(gp, "plot for [j=2:10] \'%s\' every ::%d::%d using 1:j with lines title word(titles ,j-1)\n",data_file7,tend*(i-1),tend*i-1);
        pclose(gp);
    }
