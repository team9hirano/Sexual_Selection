...
void Map(const char *sex, const char *filename, double K, double initP, int t)
{
    ...
    if (strcmp(sex, "male") == 0)
    {
        if (strstr(filename, "stmap"))
            ...
            fprintf(gp, "set output 'Small_map/MapInt_%s_K_%g_initT2P2_%f_t_%06d.png'\n", sex, K, initP, t);
...
int main(void)
{
    ...
    RecordT2P2 *buffer = malloc(sizeof(RecordT2P2) * 10 * (tend + 1) * N_SEED);
    int buf_count = 0;
    // data7(遺伝子頻度の書き込み用)
    typedef struct
    {
    ...
    Recordgenoport *genorepo = malloc(sizeof(Recordgenoport) * 10 * (tend + 1) * N_SEED);
    int geno_count = 0;
    ...
    Localpair *Pair = malloc(sizeof(Localpair) * 10 * (tend + 1) * N_SEED);
    int pair_count = 0;
    Localpair *Pair_freq = malloc(sizeof(Localpair) * 10 * (tend + 1) * N_SEED);
    int freq_count = 0;
    ...
    {
        int situ;
        double k, u, gen1, gen2, gen3, gen4, initT2P2;
    } Threshold;
    Threshold *Threshold1 = malloc(sizeof(Threshold) * 10*10*5*N_SEED);
    ...
    for (iK = 1; iK <= 9; iK++)
    {
        ...
        for (iu = 0; iu <= 8; iu++) // 20
        {
            ...
            // sprintf(data_file1, "Small_K_%f_u_%f.dat", K, u);

            // data_file2 = malloc(100);
            // sprintf(data_file2, "Small_flow_K_%f_u_%f.dat", K, u);

            // data_file4 = malloc(100);
            // sprintf(data_file4, "Small_pair_K_%f_u_%f.dat", K, u);

            // data_file5 = malloc(100);
            // sprintf(data_file5, "Small_pair_flow_K_%f_u_%f.dat", K, u);

            // data_file6 = malloc(100);
            // sprintf(data_file6, "Small_pair_final_K_%f_u_%f.dat", K, u);

            data_file7 = malloc(100);
            sprintf(data_file7, "Small_genoport_K_%f_u_%f.dat", K, u);

            // data_file8 = malloc(100);
            // sprintf(data_file8, "Small_pair_full_K_%f_u_%f.dat", K, u);
            ...
            for (k = 1; k <= 1; k++)
            {
                ...
                for (k2 = 1; k2 <= 5; k2++) // k2=1; k2<=9; k2++
                {
                    ...
                    for (int seed = 0; seed < N_SEED; seed++)
                    {
                        ...
                        // 乱数だけ変える
                        init_genrand(1234u + seed);
                        for (int tid = 0; tid < num_threads; tid++)
                        {
                            init_genrand_mt(&rng_states[tid],
                                            5489UL + (unsigned long)seed * 1000UL + (unsigned long)tid * 12345UL);
                        }

                        Initialize(initT2P2, maleT, maleP, femaleT, femaleP);
...
#pragma omp for collapse(2) schedule(static)
                            ...
                            //     sprintf(snapshot_file2, "Small_intmap_t_%d_K_%f_u_%f_initT2P2_%d.dat", t, K, u, initT2P2);
                            ...
                            //     }
                            //     fclose(snapshot2);
                            //     Map("male", snapshot_file2, K, (double)initT2P2, t);
                            //     Map("female", snapshot_file2, K, (double)initT2P2, t);
                            // }
                            ...
                            if (t == tend)
                            {
                                sum1 = (double)sum1 / (double)(LH * LV);
                                sum2 = (double)sum2 / (double)(LH * LV);
                                sum3 = (double)sum3 / (double)(LH * LV);
                                sum4 = (double)sum4 / (double)(LH * LV);
                                if (fabs(sum1 - 0.0) < 1e-6)
                                {
                                    situ = 1;
                                }
                                else if (fabs(sum4 - 0.0) < 1e-6)
                                {
                                    situ = 2;
                                }
                                else
                                ...
                                Threshold1[Threshold_count].initT2P2 = initT2P2;
                                Threshold1[Threshold_count].situ = situ;
                                Threshold1[Threshold_count].gen1 = sum1;
                                Threshold1[Threshold_count].gen2 = sum2;
                                Threshold1[Threshold_count].gen3 = sum3;
                                Threshold1[Threshold_count].gen4 = sum4;
                                Threshold_count = Threshold_count + 1;
                                printf("K=%f u=%f 周りのT2P2の数=%d situ=%d T1P1=%f T2P2=%f\n", K, u, initT2P2, situ,
                                       sum1, sum4);
                            }
                        }
                    }
            ...
            // sprintf(data_file3, "Small_final_K_%f_u_%f.dat", K, u);
            ...
            for (i = 1; i <= 5; i++)
            {
                ...
                gp = popen("gnuplot -persist", "w");
                fprintf(gp, "set terminal png\n");
                fprintf(gp, "set output 'Small_geno/Small_genoport_K_%f_u_%f_a1_%f_initT2P2_%d.png'\n", K, u, a1, initT2P2);
                fprintf(gp, "set xrange [0:%d]\n", tend);
    ...
    gp = fopen(data_file9, "w");
        fprintf(gp, "K,u,situ,initT2P2,freq_T1P1,freq_T1P2,freq_T2P1,freq_T2P2\n");
        for (i = 0; i < Threshold_count; i++)
        {
            fprintf(gp, "%f,%f,%d,%f,%f,%f,%f,%f\n", Threshold1[i].k, Threshold1[i].u, Threshold1[i].situ, Threshold1[i].initT2P2, Threshold1[i].gen1, Threshold1[i].gen2, Threshold1[i].gen3, Threshold1[i].gen4);
        }
        fclose(gp);
        // Threshold_count = 0;
    free(data_file9);
