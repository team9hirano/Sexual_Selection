...
int main(void)
{
    ...
    for (iK = 0; iK <= 0; iK++)
    {
        ...
        K = 0.01 + (double)iK * 0.01;
        s1=s2=curs1=curs2=curs3=2.0;situ=0;
        printf("K:%f\n", K);
        count=0;
        for(iu=0;iu<=100;iu++){
            if(count==0){
                ...
                else if(iu==1 && situ==2){//U=0で共存で終了
                    s1=0.0;
                    curs2=0.0;
                    count=1;
                    continue;
                }
                else if(iu==1&&situ==3){//U=0で絶滅(T1P1のみ)で終了
                    s1=0.0;
                    s2=0.0;
                    curs3=0.0;
                    count=1;
                ...
                else if(fabs(curs2-2.0)<0.00001 && curs1<1.0){//１回も共存していないため共存状態を探す
                    u=2.0*u;
                }
                else if(fabs(curs2-2.0)<0.00001){//共存状態がなかったので終了
                    s1=curs1;
                    count=1;
                    continue;
                }
                else if(situ==1){//uではT2P1orT2P2のみである場合
                    curs1=u;
                    u=(curs1+curs2)/2.0;
                }
                else if(situ==2){//uで共存の場合
                    curs2=u;
                    u=(curs1+curs2)/2.0;
                }
                else if(situ==3){//uで絶滅である場合
                    u=(curs1+u)/2.0;
                }
                else if(fabs(curs2-curs1)<0.01){//十分に近づいた場合閾値発見で終了
                    s1=curs1;
                ...
                if(fabs(curs3-0.0)<0.00001){//絶滅閾値発見で終了
                    break;
                }
                else if(fabs(curs2-2.0)<0.00001){//共存状態がなかったので終了
                    s2=1.0;break;
                }
                else if(iu==0){//最初の値を設定
                    u=1.0;
                }
                else if(iu==1 && fabs(curs2-1.0)<0.00001){//u=1.0で共存で終了
                    curs2=u;
                    s2=1.0;
                    break;
                }
                else if(situ==3){//uで絶滅の場合
                    curs3=u;
                    u=(curs2+curs3)/2.0;
                }
                else if(situ==2){//uで共存の場合
                ...
                else if(fabs(curs3-curs2)<0.01){//十分に近づいた場合閾値発見で終了
                    s2=curs3;
...
#pragma omp for collapse(2) schedule(static)
        ...
        if(fabs(gen3-1.0)<1e-9 || fabs(gen4-1.0)<1e-9 ){
