...
#define tend 50000 // 4000 80000 10000 70000 30000
...
int main(void)
{
    ...
    {
        int situ;
        double k, u, gen1, gen2, gen3, gen4, initT2P1;
    } Threshold;
    Threshold *Threshold1 = malloc(sizeof(Threshold) * 5000);
                ...
                for (int seed = 0; seed < N_SEED; seed++) {
                    ...
                    for (int tid = 0; tid < num_threads; tid++) {
                        init_genrand_mt(&rng_states[tid],
                            5489UL + (unsigned long)seed * 1000UL
                                + (unsigned long)tid * 12345UL);
                    }
...
#pragma omp for collapse(2) schedule(static)
                    ...
                    for (i = 0; i < 6; i++)
                        ...
                        if (t == tend)
                        {
                            ...
                            if ((fabs(sum1 - 0.0) < 1e-6 && fabs(sum2 - 0.0) < 1e-6 && fabs(sum3 - 0.0) < 1e-6) || (fabs(sum1 - 0.0) < 1e-6 && fabs(sum2 - 0.0) < 1e-6 && fabs(sum4 - 0.0) < 1e-6))
                            {
                                // 全部同じ遺伝子型になった場合、流れを出力しない
                                situ = 1;
                            }
                            else if (fabs(sum1 - 1.0) < 1e-6)
                            {
                            ...
                            Threshold1[Threshold_count].initT2P1 = initT2P1;
                            ...
                            printf("K=%f u=%f initT2P1=%f situ=%d T1P1=%f T1P2=%f T2P1=%f T2P2=%f\n", K, u, initT2P1, situ,
                                   sum1, sum2, sum3, sum4);
                        }
                    }
                }
    ...
    fprintf(gp, "K,u,situ,initT2P1,freq_T1P1,freq_T1P2,freq_T2P1,freq_T2P2\n");
    for (i = 0; i < Threshold_count; i++)
    {
        fprintf(gp, "%f %f %d %f %f %f %f %f\n", Threshold1[i].k, Threshold1[i].u, Threshold1[i].situ, Threshold1[i].initT2P1, Threshold1[i].gen1, Threshold1[i].gen2, Threshold1[i].gen3, Threshold1[i].gen4);
    }
    fclose(gp);
    Threshold_count=0;
    free(data_file9);
