...
#define tend 300000 // 4000 80000 10000 80000
#define mapinitP 0.5
#define initialP 3
#define initialT 1
        ...
        for (k2 = 1; k2 <= 9; k2++) // k2=1; k2<=9; k2++
        {
            ...
            for(t=0;t<tend;t++)
            {
                if (t % 10 == 0){
                    ...
                    buffer[buf_count].sum1 = (double)sum1 / (double)(LH * LV);buffer[buf_count].sum2 = (double)sum2 / (double)(LH * LV);
                    buffer[buf_count].sum3 = (double)sum3 / (double)(LH * LV);buffer[buf_count].sum4 = (double)sum4 / (double)(LH * LV);
...
#pragma omp for collapse(2) schedule(static)
    ...
    pclose(gp);
     ...
     //T2P1-T3P3図
    data_file3 = malloc(100);
    sprintf(data_file3, "Three_env_final_T2P1_T3P3_K_%f_V_%f_l_%f_a1_%f_a2_%f.dat", K,V,l,a1,a2);
    // data_file3 = "final_env.dat";
    gp = fopen(data_file1, "r");
    data3 = fopen(data_file3, "w");
    while (fscanf(gp, "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", &x1, &sum1, &sum2,&sum3,&sum4,&sum5,\
