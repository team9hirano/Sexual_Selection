...
int main(void)
{
    ...
    RecordT2P2 *buffer = malloc(sizeof(RecordT2P2) * 10 * (tend + 1));
    int buf_count = 0;
    // data7(遺伝子頻度の書き込み用)
    typedef struct
    {
    ...
    Recordgenoport *genorepo = malloc(sizeof(Recordgenoport) * 10 * (tend + 1));
    int geno_count = 0;
    ...
    Localpair *Pair = malloc(sizeof(Localpair) * 10 * (tend + 1));
    int pair_count = 0;
    Localpair *Pair_freq = malloc(sizeof(Localpair) * 10 * (tend + 1));
    int freq_count = 0;
