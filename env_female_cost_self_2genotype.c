...
void Map(const char *sex, const char *filename, double initP, int t)
{
    ...
    if (strcmp(sex, "male") == 0)
    {
        if (strstr(filename, "stmap"))
            fprintf(gp, "set output 'Twogenotyperesult/TwoMapSt_T1P1vsT2P2_%s_env_cost_self_%g_initP_%g.png'\n", sex, K, initP);
        else if (strstr(filename, "intmap"))
            fprintf(gp, "set output 'Twogenotyperesult/TwoMapInt_T1P1vsT2P2_%s_env_cost_self_%g_initP_%g_t_%d.png'\n", sex, K, initP, t);
        else if (strstr(filename, "finmap"))
            fprintf(gp, "set output 'Twogenotyperesult/TwoMapFin_T1P1vsT2P2_%s_env_cost_self_%g_initP_%g.png'\n", sex, K, initP);
...
int main(void)
{
    ...
    sprintf(data_file1, "Twogenotype_%f.dat", K);

    data_file2 = malloc(100);
    sprintf(data_file2, "Twoflow_%f.dat", K);
    ...
    for (k = 1; k <= 1; k++)
    {
        ...
        for (k2 = initialP; k2 <= initialP; k2++) // k2=1; k2<=9; k2++
        {
            ...
            while (t < tend)
            {
                ...
                { // t<tend
                    sprintf(snapshot_file2, "Twogen_intmap_t_%d_cost_%f_initP_%g.dat", t, K, mapinitP);
    ...
    sprintf(data_file3, "Twofinal_%f.dat", K);
    ...
    gp = popen("gnuplot -persist", "w");
    fprintf(gp, "set terminal png\n");
    fprintf(gp, "set output 'Twogenotyperesult/Twogenotype_%f.png'\n", K);
